...
public readonly ref struct LuauStringView
{
    ...
    internal LuauStringView(LuauState? state, int stackIndex, int callbackFrameToken)
    {
        _source = LuauRefSource.FromCallbackStack(state, stackIndex, callbackFrameToken, lua_Type.LUA_TSTRING);
    }
    ...
    public bool TryGet(out ReadOnlySpan<byte> value) =>
        LuauStringAccessCore.TryGet(_source, nameof(LuauStringView), out value);
    ...
    public static implicit operator IntoLuau(LuauStringView value) => IntoLuau.FromRefSource(value._source);
